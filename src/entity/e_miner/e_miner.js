...
class ElectricalMiner extends Inventory {
    ...
    setup(map, ent) {
        ...
        this.energy = 0;
        this.power = 0;
        this.mapsize = {x: c.resDB.e_miner.size[0], y: c.resDB.e_miner.size[1]};
    }
    ...
    updateNB() {
        ...
        for(let x = scanArea.x; x < scanArea.x2; x++) {
            ...
            if (nb?.type == c.resDB.pole.id && this.nbInputs.includes(nb.id) == false) this.nbInputs.push(nb.id);
            }
        }
    }
...
db.size = [2, 2];
exports.ElectricalMiner = ElectricalMiner;